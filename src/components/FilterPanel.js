...
export default function FilterPanel({ dispatchProductsURI }) {
  return (
    <section>
              ...
              <div className="accordion-body">
                <section className="mb-4" data-filter="condition">
                  <div className="form-check mb-3">
                    <input className="form-check-input" type="radio" name="category" id={`price-radio0`} onClick={() => {
                      dispatchProductsURI({ type: 'ResetURICategory' })
                      dispatchProductsURI({ type: 'ResetURISkip' })
                    }} />
                    <label className="form-check-label text-uppercase small" htmlFor={`price-radio0`}>
                      All
                    </label>
                  </div>
                  {[
                    ...
                    <div key={i} className="form-check mb-3">
                      <input className="form-check-input" type="radio" name="category" id={`price-radio${i + 1}`} onClick={() => {
                        dispatchProductsURI({ type: 'UpdateURICategory', payload: category })
                        dispatchProductsURI({ type: 'ResetURISkip' })
                      }} />
                      <label className="form-check-label text-uppercase small" htmlFor={`price-radio${i}`}>
                        {category}
                      </label>
                    </div>
                  )
                    ...
                    [4, 3, 2, 1].map((ratings, i) =>
                      <div key={i} className="form-check mb-3">
                        ...
                        <label className="form-check-label small" htmlFor={`ratings-radio${i}`}>
                          {Array(ratings).fill().map(() => <i className="fas fa-star text-warning"></i>)} and up
                        </label>
                      </div>
                    )
                  }
