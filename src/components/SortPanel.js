...
export default function SortPanel({ dispatchProductsURI }) {
  const handleChange = useCallback(
    (e) => {
      if (e.target.value === 'priceAscending') {
        dispatchProductsURI({ type: 'SortByPriceAscending' })
      } else if (e.target.value === 'priceDescending') {
        dispatchProductsURI({ type: 'SortByPriceDescending' })
      } else if (e.target.value === 'ratingDescending') {
        dispatchProductsURI({ type: 'SortByRatingDescending' })
      }
    },
    [],
  )
